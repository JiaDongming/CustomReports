...
namespace BLL
{
    public class ReportManager
    {
        ...
        #region -----------------------以下为真实客户报表luzhiwei------------------------------------------


        #region 22-第22个报表业务逻辑方法
        protected List<Report22_ProjectSpace> GetReport22_Project()
        {
            SqlDataReader ReportData = objReportService.GetReport22_Project();
            List<Report22_ProjectSpace> ReportDataList = new List<Report22_ProjectSpace>();
            while (ReportData.Read())
            {
                ReportDataList.Add(new Report22_ProjectSpace()
                {
                    ProjectID = ReportData["ProjectID"].ToString(),
                    SubProjectID = ReportData["SubProjectID"].ToString(),
        ...
        protected static List<Report22_Bug> GetReport22_ReportData()
        {
            SqlDataReader ReportData = ReportService.GetReport22_ReportData();
            List<Report22_Bug> ReportDataList = new List<Report22_Bug>();

            while (ReportData.Read())
            {
               
                ReportDataList.Add(new Report22_Bug()
                {
                    ProjectID = ReportData["ProjectID"].ToString(),
                    SubProjectID = ReportData["SubProjectID"].ToString(),
                    BugID = ReportData["BugID"].ToString(),
                    BugTitle= ReportData["BugTitle"].ToString(),
                    PlanedEndDate = ReportData["PlanedEndDate"] ==DBNull.Value?"无":ReportData["PlanedEndDate"].ToString(),
                    IssueFinishDate = ReportData["IssueFinishDate"]==DBNull.Value?"无": ReportData["IssueFinishDate"].ToString(),
                    PlanedEndDate2 = ReportData["PlanedEndDate"] == DBNull.Value ? Convert.ToDateTime("1970-01-01"):Convert.ToDateTime( ReportData["PlanedEndDate"]),
                    IssueFinishDate2 = ReportData["IssueFinishDate"] == DBNull.Value ? Convert.ToDateTime("1970-01-01") : Convert.ToDateTime(ReportData["IssueFinishDate"])
                });
            }
            ReportData.Close();
            return ReportDataList;
        ...
        {
           return  ReportService.DeleteTmpData();
        }
        public static void insertTmpData(string selectSpaceID)
        {
            ReportService.insertTmpData(selectSpaceID);
        }
        ...
        /// <param name="multiSelectProjectChoice"></param>
        /// <returns></returns>
        public static string GetReport22_HTML()
        {
            List<Report22_Bug> reportDataList = GetReport22_ReportData();
            if (reportDataList.Count == 0)
                return string.Empty;

            //写报表头部
            StringBuilder sbReportHTML = new StringBuilder(@"<table class='report_table' style='width:100%; text-align:left' ><tr>");
            foreach (Report22_Bug item in reportDataList)
            {
                sbReportHTML.AppendFormat("<th><p>{0} </p> <span class='finishtime'> 计划完成时间，实际完成时间</span></th>",item.BugTitle);
            }
           
            sbReportHTML.Append("</tr><tbody><tr>");

            foreach (Report22_Bug item in reportDataList)
            {
                if (item.IssueFinishDate2< item.PlanedEndDate2 && item.IssueFinishDate2 != Convert.ToDateTime("1970-01-01"))//过期(实际完成时间大于计划完成时间)
                {
                    sbReportHTML.AppendFormat("<td> <span>{0}<span>，<span class='red'>{1}</span></td>", item.PlanedEndDate, item.IssueFinishDate);
                    continue;
                }
                //过期(实际完成时间为空，且当前系统时间大于计划完成时间)
                if (item.IssueFinishDate2 == Convert.ToDateTime("1970-01-01") && DateTime.Now > item.PlanedEndDate2 && item.PlanedEndDate2!= Convert.ToDateTime("1970-01-01"))
                {
                    sbReportHTML.AppendFormat("<td> <span  class='red'>{0}<span>，<span>{1}</span></td>", item.PlanedEndDate, item.IssueFinishDate);
                    continue;
                }

                sbReportHTML.AppendFormat("<td> <span>{0}<span>，<span>{1}</span></td>", item.PlanedEndDate, item.IssueFinishDate);
